...
├── greeter_application/            # <-- Your application directory (not generated)
│   ├── __init__.py
│   └── greeter.py
│
├── greeter_fastworkflow/           # <-- The workflow_folderpath (generated)
│    |
...
│
├── env/                            # <-- Project environment (copy from hello_world example)
│    └── fastworkflow.env
│
└── passwords/                      # <-- Project passwords (copy from hello_world example)
     └── fastworkflow.passwords.env
```
-   Your application code (`greeter_application/`) remains untouched.
-   The `fastWorkflow` definition (`greeter_fastworkflow/`) contains all the generated files and trained models. The build tool (`--app-dir`) points to your app code, while the output flag (`--workflow-folderpath`) points to the workflow directory. Think of it as the *UI layer* that fastWorkflow builds **around** your core logic.
...
```sh
mkdir -p my_project/greeter_application
```

### Step 3: Create a new workflow directory

```sh
mkdir -p my_project/greeter_fastworkflow
# "__init__.py" marks the folder as a Python package so imports work
touch my_project/greeter_fastworkflow/__init__.py
```

### Step 4: Design Your Application

Create a simple Python class in `my_project/greeter_application/greeter.py`.
...
Create a file named `my_project/greeter_fastworkflow/_commands/Greeter/greet.py`. This file tells `fastWorkflow` how to handle the `greet` command for the `Greeter` context.

```python
# my_project/greeter_fastworkflow/_commands/Greeter/greet.py
import fastworkflow
from pydantic import BaseModel, Field
from greeter_application.greeter import Greeter  # NEW import so type hints resolve
...
### Step 6: Create the Context Model

Create `my_project/greeter_fastworkflow/context_inheritance_model.json`. This file defines the contexts and their inheritance. For our simple case, it's just the `Greeter`.
...
### Step 7: Train and Run

Your manual workflow is ready!
```sh
# Train the workflow
fastworkflow train my_project/greeter_fastworkflow/ fastworkflow.env fastworkflow.passwords.env

# Run the workflow (omit --startup_command if you haven’t created startup.py yet)
fastworkflow run my_project/greeter_fastworkflow/ fastworkflow.env fastworkflow.passwords.env
```
*(If you want the workflow to greet you automatically, create a minimal `startup.py` command and then add `--startup_command startup`.)*
...
Update `my_project/greeter_fastworkflow/context_inheritance_model.json` to reflect the inheritance:
```json
{
  "Greeter": {
    "base": []
...
Create a file named `my_project/greeter_fastworkflow/_commands/Greeter/_Greeter.py`:
```python
# my_project/greeter_fastworkflow/_commands/Greeter/_Greeter.py

...
'''
    ...
    def test_full_workflow_processing(self, mock_configure, mock_openai):
        ...
        with patch.object(GenAIPostProcessor, '_initialize_dspy'), \
            ...
            # Create processor
            processor = GenAIPostProcessor(model="gpt-4", api_key="test_key")
    ...
    def _verify_command_file_updated(self, file_path):
        ...
        if sig_class.body and isinstance(sig_class.body[0], ast.Expr):
            if isinstance(sig_class.body[0].value, ast.Constant):
                # Docstring was added
    ...
    @patch('dspy.OpenAI')
    @patch('dspy.settings.configure')
    def test_error_recovery(self, mock_configure, mock_openai):
    ...
    @patch('dspy.OpenAI')
    @patch('dspy.settings.configure')
    def test_caching_behavior(self, mock_configure, mock_openai):
    ...
    def test_command_file_ast_preservation(self):
        ...
        # Create processor with mocked DSPy
        with patch('dspy.OpenAI'), patch('dspy.settings.configure'):
            processor = GenAIPostProcessor()
        
        # Update the AST
        enhanced_input = [{'name': 'param1', 'type': 'str', 'description': 'Test', 'examples': [], 'constraints': ''}]
        enhanced_output = [{'name': 'result', 'type': 'bool', 'description': 'Test', 'examples': [], 'constraints': ''}]
...
class TestPostProcessorCLIIntegration(unittest.TestCase):
    ...
    def test_cli_arguments_parsing(self):
        ...
        with patch('sys.argv', ['prog', '--app-dir', '/app', '--workflow-folderpath', '/workflow',
                               '--no-genai']):
            args = parse_args()
            self.assertTrue(args.no_genai)
            self.assertEqual(args.app_dir, '/app')
            self.assertEqual(args.workflow_folderpath, '/workflow')
    
    def test_cli_genai_model_argument(self):
        """Test GenAI model argument parsing."""
        from fastworkflow.build.__main__ import parse_args
        
        with patch('sys.argv', ['prog', '--app-dir', '/app', '--workflow-folderpath', '/workflow',
                               '--genai-model', 'gpt-3.5-turbo']):
            args = parse_args()
            self.assertEqual(args.genai_model, 'gpt-3.5-turbo')
    
    def test_cli_genai_api_key_argument(self):
        """Test GenAI API key argument parsing."""
        from fastworkflow.build.__main__ import parse_args
        
        with patch('sys.argv', ['prog', '--app-dir', '/app', '--workflow-folderpath', '/workflow',
                               '--genai-api-key', 'test-key-123']):
            args = parse_args()
            self.assertEqual(args.genai_api_key, 'test-key-123')
    ...
    @patch('fastworkflow.build.genai_postprocessor.run_genai_postprocessor')
    def test_build_flow_with_no_genai_flag(self, mock_run_postprocessor):
        """Test that post-processor is skipped with --no-genai flag."""
        from fastworkflow.build.__main__ import run_command_generation
        
        # Create mock args with no_genai flag
        args = MagicMock()
        args.app_dir = "/test/app"
        args.workflow_folderpath = "/test/workflow"
        args.overwrite = False
        args.stub_commands = None
        args.no_startup = False
        args.no_genai = True  # Disable GenAI
        ...
        with patch('fastworkflow.build.__main__.ast_class_extractor.analyze_python_file') as mock_analyze, \
            ...
            # Verify post-processor was NOT called
            mock_run_postprocessor.assert_not_called()

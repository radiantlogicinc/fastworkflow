...
class TestDSPySignatures(unittest.TestCase):
    ...
    def test_field_metadata_signature(self):
        ...
        self.assertIn('constraints', sig.__annotations__)
    ...
    def test_utterance_generator_signature(self):
        ...
        self.assertIn('input_fields', sig.__annotations__)
    ...
    def test_signature_docstring_signature(self):
        ...
        self.assertIn('command_name', sig.__annotations__)
        self.assertIn('input_fields', sig.__annotations__)
        self.assertIn('output_fields', sig.__annotations__)
        self.assertIn('context_name', sig.__annotations__)
    ...
    def test_context_docstring_signature(self):
        ...
        # Check input fields
        self.assertIn('context_name', sig.__annotations__)
        self.assertIn('commands', sig.__annotations__)
    ...
    def test_workflow_description_signature(self):
        ...
        # Check input fields
        self.assertIn('contexts', sig.__annotations__)
        self.assertIn('global_commands', sig.__annotations__)
...
class TestDSPyModules(unittest.TestCase):
    ...
    def test_field_metadata_generator(self, mock_cot):
        ...
        mock_result.constraints = "must be positive"
        ...
        self.assertEqual(result.constraints, "must be positive")
...
class TestGenAIPostProcessor(unittest.TestCase):
    ...
    def test_initialization(self, mock_configure, mock_openai):
...
"""
        ...
        for node in class_node.body:
            if isinstance(node, ast.AnnAssign) and isinstance(node.target, ast.Name):
                if node.target.id == 'field1':
                    # Should have Field() call with metadata
...
"""
    ...
    def test_generate_workflow_description(self):
        ...
        # Create processor (with mocked DSPy and modules)
        with patch('dspy.OpenAI'), patch('dspy.settings.configure'):
            processor = GenAIPostProcessor()
...
class TestIntegrationFunction(unittest.TestCase):
    ...
    def test_run_genai_postprocessor_disabled(self):
        """Test that post-processor can be disabled via flag."""
    ...
    def test_run_genai_postprocessor_no_api_key(self):
        """Test handling of missing API key."""
        # Create mock args without API key
        args = MagicMock()
        args.no_genai = False
        args.genai_model = "gpt-4"
        args.genai_api_key = None
        args.workflow_folderpath = "/test/path"
    ...
    def test_run_genai_postprocessor_success(self, mock_processor_class):
        ...
        # Verify
        self.assertTrue(result)
        mock_processor_class.assert_called_once_with(model="gpt-4", api_key="test_key")

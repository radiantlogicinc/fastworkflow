...
- CI runs only `tests/test_train_modern_stack.py` (single workflow `.github/workflows/train-modern-stack.yml`) — so a removed test will NOT be caught by CI. The full suite is local-only: `source .venv/bin/activate && python -m pytest`, and it takes the better part of an hour, so budget it rather than backgrounding it and hoping. Run it before release commits; Rule 3 applies to how training tests isolate themselves. The current test count and full-run baseline live in `fastworkflow-validation-and-qa` and are deliberately not repeated here.
...
- There IS a sanctioned path for *public* skills: `fastworkflow/skills_for_coding_fastworkflows/` (integrate-chat-agent was deliberately committed there via `1d3a6aa` at its original home `fastworkflow/docs/integrate-chat-agent/`, and the whole public coding-skills library moved to this directory 2026-08-25). Public skill ⇒ that directory, with approval. Team-private skill ⇒ `.claude/skills/`, uncommitted.

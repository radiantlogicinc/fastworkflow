...
def _collect_command_params(workflow_path: str) -> Dict[str, CommandParams]:
    ...
    for qualified_name in directory.get_commands():
        ...
        # Respect that only commands present in routing are considered
        contexts = routing.get_contexts_for_command(qualified_name)
        if contexts:  # only register if command is routable somewhere
...
def _semantic_match_score(out_param: ParamMeta, in_param: ParamMeta) -> float:
    # Lightweight semantic similarity using simple token overlap Jaccard
    ...
    a = set(to_text(out_param).split())
    b = set(to_text(in_param).split())
    if not a or not b:
        return 0.0
    inter = len(a & b)
    union = len(a | b)
...
def get_dependency_suggestions(
    ...
    def recurse(node: str, depth: int) -> List[Dict[str, Any]]:
        ...
        for e in adj.get(node, []):
            ...
            if not any(mp[0] == missing_input_param for mp in matched_params):
                continue
            neighbor = e["to"]

    ...
    def validate_command_metadata_map(cls, map_commands_2_metadata):
        ...
        for command_name, metadata in map_commands_2_metadata.items():
            ...
            if isinstance(metadata, dict):
                ...
                raise ValueError(f"Invalid value for command metadata for '{command_name}'")
...
class RoutingFailureFallbackDefinition(BaseModel):
    ...
    @model_validator(mode='after')
    def validate_routing_failure_fallback_definition(self) -> 'RoutingFailureFallbackDefinition':
        if (self.fallback_workflow and self.fallback_command) or (not self.fallback_workflow and not self.fallback_command):
            raise ValueError("Exactly one of fallback workflow or fallback command must be populated.")
...
class CommandRoutingDefinition(BaseModel):
    ...
    map_workitem_types_2_commandexecutionmetadata: dict[str, CommandExecutionMetadata] = None

    @field_validator('map_workitem_types_2_commandexecutionmetadata', mode='before')
    def validate_command_routing_map(cls, map_workitem_types_2_commandexecutionmetadata):
        if not map_workitem_types_2_commandexecutionmetadata:
            raise ValueError("map_workitem_types_2_commandexecutionmetadata cannot be empty")

        for workitem_type, command_execution_metadata in map_workitem_types_2_commandexecutionmetadata.items():
            ...
            if isinstance(command_execution_metadata, dict):
                map_workitem_types_2_commandexecutionmetadata[workitem_type] = CommandExecutionMetadata(**command_execution_metadata)
            elif not isinstance(map_workitem_types_2_commandexecutionmetadata[workitem_type], CommandExecutionMetadata):
                raise ValueError(f"Invalid value for command execution metadata for '{workitem_type}'")

        return map_workitem_types_2_commandexecutionmetadata

    @classmethod
    def create(cls, workflow_folderpath: str) -> 'CommandRoutingDefinition':
        map_workitem_types_2_commandexecutionmetadata: dict[str, CommandExecutionMetadata] = {}
    ...
    def get_command_names(self, workitem_type: str) -> list[str]:
        if workitem_type in self.map_workitem_types_2_commandexecutionmetadata:
            return list(self.map_workitem_types_2_commandexecutionmetadata[workitem_type].map_commands_2_metadata.keys())
        else:
            raise ValueError(f"Command routing definition not found for workitem type '{workitem_type}'")


    def get_command_class(self, 
                        workitem_type: str, 
                        command_name: str, 
                        module_type: ModuleType) -> Optional[Type[Any]]:
        if workitem_type in self.map_workitem_types_2_commandexecutionmetadata:
            command_execution_metadata = self.map_workitem_types_2_commandexecutionmetadata[workitem_type]
            if command_name in command_execution_metadata.map_commands_2_metadata:
                command_metadata = command_execution_metadata.map_commands_2_metadata[command_name]
                ...
                workflow_folder_syspath = f"{self.workflow_folderpath}/" if not self.workflow_folderpath.endswith('/') else ...
                relative_module_filepath = os.path.relpath(module_file_path, os.path.dirname(workflow_folder_syspath))

                package_name = os.path.dirname(relative_module_filepath).replace('/', '.').replace('\\', '.')
                ...
                raise ValueError(f"Command '{command_name}' not found for workitem type '{workitem_type}'")
        else:
            raise ValueError(f"Command routing definition not found for workitem type '{workitem_type}'")

    def get_command_class_object(self, 
                                 workitem_type: str, 
                                 command_name: str, 
                                 module_type: ModuleType) -> Optional[Type[object]]:
    ...
    def _populate_command_execution_metadata(
        ...
        if parent_workitem_type:
            command_execution_metadata = map_workitem_types_2_commandexecutionmetadata[parent_workitem_type]
            for command, command_metadata in command_execution_metadata.map_commands_2_metadata.items():
        ...
        if os.path.exists(commands_folder):
            for command in os.listdir(commands_folder):
                ...
                parameter_extraction_folderpath = os.path.join(subfolder_path, "parameter_extraction")
                if os.path.exists(parameter_extraction_folderpath):
                    signature_filepath = os.path.join(parameter_extraction_folderpath, 'signatures.py')
                ...
                response_generation_folderpath = os.path.join(subfolder_path, "response_generation")
                if not os.path.exists(response_generation_folderpath):
                    raise ValueError(f"Response generation folder not found at '{response_generation_folderpath}'")
                inference_filepath = os.path.join(response_generation_folderpath, 'inference.py')
                if not os.path.exists(inference_filepath):
                    raise ValueError(f"Response generation inference file not found at '{inference_filepath}'")
        ...
        if workitem_type in map_workitem_types_2_commandexecutionmetadata:
            command_execution_metadata = map_workitem_types_2_commandexecutionmetadata[workitem_type]
            for command, command_metadata in map_commands_2_metadata.items():
                command_execution_metadata.map_commands_2_metadata[command] = command_metadata
        else:
            if map_commands_2_metadata:
                map_workitem_types_2_commandexecutionmetadata[workitem_type] = CommandExecutionMetadata(

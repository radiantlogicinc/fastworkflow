...
def extract_command_parameters(
    ...
    extraction_failure_workflow: Optional[str] = None) -> Tuple[bool, BaseModel]:
    try:
    ...
    is_valid, error_msg = input_for_param_extraction.validate_parameters(session, command_parameters_obj)
    if is_valid:
        return (abort_command, command_parameters_obj)
    ...
    abort_command = command_output.payload["abort_command"] if "abort_command" in command_output.payload else False
    if abort_command:

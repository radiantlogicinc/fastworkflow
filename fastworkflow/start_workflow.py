...
def start_workflow(workflow_path: str, 
                   startup_command: str = "",
                   payload: Optional[dict] = None, 
                   keep_alive=True) -> CommandOutput:
    # Initialize colorama
    init(autoreset=True)
    ...
    if startup_command:
        command_output: CommandOutput = command_router.route_command(
            ...
            startup_command, payload)
        print(f"{Fore.GREEN}{Style.BRIGHT}{session.root_workitem_type.upper()} AI>{Style.RESET_ALL}{Fore.GREEN} {command_output.response}{Style.RESET_ALL}")

    while not session.workflow.is_complete or keep_alive:
        user_command = input(f"{Fore.YELLOW}{Style.BRIGHT}User>{Style.RESET_ALL}{Fore.YELLOW} ")
        command_output: CommandOutput = command_router.route_command(
            map_workitem_type_2_route_layer,
            user_command)

        abort_command = command_output.payload.get("abort_command", False) if command_output.payload else False
        if abort_command:
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--startup_command", help="Optional startup command", default="")
    ...
    print(f"{Fore.GREEN}{Style.BRIGHT}AI>{Style.RESET_ALL}{Fore.GREEN} AI Assistant is running with workflow: {args.workflow_path}{Style.RESET_ALL}")
    print(f"{Fore.GREEN}{Style.BRIGHT}AI>{Style.RESET_ALL}{Fore.GREEN} Type 'exit' to quit.{Style.RESET_ALL}")

    command_output = start_workflow(args.workflow_path, 

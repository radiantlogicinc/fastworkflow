...
class CommandExecutor:
    ...
    def invoke_command(self, workitem_type: str, command_name: str, 
                       command: str, payload: Optional[dict] = None) -> CommandOutput:
        ...
        response_generation_object = self._session.command_routing_definition.get_command_class_object(
            workitem_type, command_name, CommandModuleType.RESPONSE_GENERATION_INFERENCE)
        if not response_generation_object:
            raise ValueError(f"Response generation object not found for workitem type '{workitem_type}' and command name '{command_name}'")

        abort_command = False
        input_obj = None
        input_for_param_extraction_class = self._session.command_routing_definition.get_command_class(
            workitem_type, command_name, CommandModuleType.INPUT_FOR_PARAM_EXTRACTION_CLASS)
        command_parameters_class = self._session.command_routing_definition.get_command_class(
        ...
        if input_obj:
            return response_generation_object(self._session, command, input_obj, payload)
        else:

import json
from typing import List, Dict
from datasets import load_dataset
from together import Together
import random
import fastworkflow

# def generate_diverse_utterances(
#     seed_utterances: List[str],
#     command_name,
#     num_personas: int = 5,
#     utterances_per_persona: int = 5
# ) -> Dict:
#     # Initialize the Together client
#     client = Together(api_key="645ab4306a057859ab62afae309cdf05b47604696fd4ae7a64d8dcff1d6c3739")

#     # Load PersonaHub dataset
#     persona_dataset = load_dataset("proj-persona/PersonaHub", data_files="persona.jsonl")['train']

#     # Randomly sample personas
#     selected_indices = random.sample(range(len(persona_dataset)), num_personas)
#     selected_personas = [persona_dataset[i]['persona'] for i in selected_indices]

#     all_generated_responses = []
#     used_personas = []

#     # Extract common themes from seed utterances
#     keywords = set()
#     for utterance in seed_utterances:
#         words = utterance.lower().split()
#         keywords.update(words)

#     utterance_patterns = list(keywords)  # Select a few key terms as guiding patterns

#     for idx, persona in enumerate(selected_personas):
#         # Create a more detailed persona prompt
#         persona_name = f"Persona_{idx + 1}"
#         used_personas.append({
#             "name": persona_name,
#             "description": persona
#         })

#         # Construct the prompt
#         utterance_string = "\n".join(seed_utterances)

#         messages = [
#             {
#                 "role": "system",
#                 "content": f"""
#                 Generate {utterances_per_persona} concise and natural utterances.

#                 Persona Description:
#                 {persona}

#                 Use these seed utterances for style and intent:
#                 {utterance_string}

#                 Guidelines:
#                 - Keep responses brief and natural
#                 - Maintain intent consistency
#                 - Avoid repeating the same structure
#                 - keep in mind this command name (intent of the utterance):-{command_name} in mind so that you generate relevent utterences.
#                 - Use varied phrasing based on these key themes: {', '.join(utterance_patterns)}
#                 - Keep only the utterance in the generated response, i.e do not give inroduction or follow up after each utterance.
#                 - DO NOT keep number in front of the sentence in response like 1.,2.
#                 """
#             },
#             {
#                 "role": "user",
#                 "content": f"Generate {utterances_per_persona} natural utterances from this persona's perspective."
#             }
#         ]


#         response = client.chat.completions.create(
#             model="meta-llama/Llama-3.3-70B-Instruct-Turbo-Free",
#             messages=messages,
#             max_tokens=1000,
#             temperature=1.0,
#             top_p=0.9,
#             top_k=50,
#             repetition_penalty=1.1,
#             stop=["<|end_of_text|>"],
#             stream=False
#         )

#         # Process responses
#         generated = response.choices[0].message.content.strip().split('\n')
#         generated = [g.strip().strip('"').strip("'") for g in generated if g.strip()]
#         generated = [g for g in generated if len(g) > 3]  # Filter out very short responses

#         all_generated_responses.extend([
#             {"utterance": resp, "persona": persona_name} for resp in generated
#         ])

#     # Structure the output
#     output = {
#         "seed_utterances": seed_utterances,
#         "generated_utterances": all_generated_responses,
#         "personas": used_personas,
#         "metadata": {
#             "num_personas": num_personas,
#             "utterances_per_persona": utterances_per_persona,
#             "total_utterances": len(all_generated_responses)
#         }
#     }

#     return output



# Example usage
# if __name__ == "__main__":
#     seed_utterances = [
#         "what are we working on?",
#         "where am I",
#         "where are we",
#         "what is our position",
#         "what is our current location",
#         "what is our current step",
#         "what is our current stage",
#         "what is our current workflow",
#         "What is the current work item?",
#         "What is the next work item?",
#         "Can you provide the name of the next work item?",
#         "get the path and id of the work item",
#         "Get the location of the next work item, skipping completed ones.",
#         "Get the location of the next work item, including the ones already completed.",
#         "get path and id"
#     ]

#     result = generate_diverse_utterances(
#         seed_utterances=seed_utterances,
#         command_name="get_id_and_path",
#         num_personas=5,
#         utterances_per_persona=5
#     )
#     all_utterances = [utt["utterance"] for utt in result["generated_utterances"]]
#     print(all_utterances)

#     with open("diverse_utterances.json", "w") as f:
#         json.dump(result, f, indent=2)

#     print(f"Generated {result['metadata']['total_utterances']} utterances using {result['metadata']['num_personas']} personas")
#     print("\nExample utterances:")
#     for i, utt in enumerate(result['generated_utterances'][:5]):
#         print(f"{i+1}. [{utt['persona']}] {utt['utterance']}")


def generate_diverse_utterances(
    seed_utterances: List[str],
    command_name,
    num_personas: int = 5,
    utterances_per_persona: int = 5,
    personas_per_batch: int = 2
) -> Dict:
    # Initialize the Together client
    client = Together(api_key="645ab4306a057859ab62afae309cdf05b47604696fd4ae7a64d8dcff1d6c3739")

    # Load PersonaHub dataset
    persona_dataset = load_dataset("proj-persona/PersonaHub", data_files="persona.jsonl")['train']

    # Randomly sample personas
    selected_indices = random.sample(range(len(persona_dataset)), num_personas)
    selected_personas = [persona_dataset[i]['persona'] for i in selected_indices]

    all_generated_responses = []
    used_personas = []

    # Extract common themes from seed utterances
    keywords = set()
    for utterance in seed_utterances:
        words = utterance.lower().split()
        keywords.update(words)

    utterance_patterns = list(keywords)
    utterance_string = "\n".join(seed_utterances)

    # Process personas in batches
    for batch_start in range(0, len(selected_personas), personas_per_batch):
        batch_end = min(batch_start + personas_per_batch, len(selected_personas))
        batch_personas = selected_personas[batch_start:batch_end]
        
        # Create combined prompt for all personas in batch
        batch_prompt = ""
        for idx, persona in enumerate(batch_personas):
            persona_name = f"Persona_{batch_start + idx + 1}"
            used_personas.append({
                "name": persona_name,
                "description": persona
            })
            batch_prompt += f"\n[{persona_name}]\n{persona}\n"

        messages = [
            {
                "role": "system",
                "content": f"""
                Generate {utterances_per_persona} unique utterances for each of the following personas.

                {batch_prompt}

                Use these seed utterances for style and intent:
                {utterance_string}

                Guidelines:
                - Generate exactly {utterances_per_persona} utterances per persona
                - Keep responses brief and natural
                - Maintain intent consistency with command: {command_name}
                - Avoid repeating the same structure
                - Use varied phrasing based on these themes: {', '.join(utterance_patterns)}
                
                Format your response exactly as follows:
                [Persona_Name]
                utterance
                utterance
                ...

                [Next_Persona_Name]
                utterance
                utterance
                ...
                """
            },
            {
                "role": "user",
                "content": f"Generate {utterances_per_persona} natural utterances for each persona listed above."
            }
        ]

        response = client.chat.completions.create(
            model="meta-llama/Llama-3.3-70B-Instruct-Turbo-Free",
            messages=messages,
            max_tokens=1000,
            temperature=1.0,
            top_p=0.9,
            top_k=50,
            repetition_penalty=1.1,
            stop=["<|end_of_text|>"],
            stream=False
        )

        # Process responses
        content = response.choices[0].message.content.strip()
        
        # Split by persona sections
        sections = content.split('[')
        for section in sections[1:]:  # Skip first empty section
            try:
                # Extract persona name and utterances
                persona_name = section.split(']')[0].strip()
                utterances = section.split(']')[1].strip().split('\n')
                
                # Clean up utterances
                utterances = [u.strip() for u in utterances if u.strip()]
                utterances = [u for u in utterances if len(u) > 3 and not u.startswith('[')]
                
                all_generated_responses.extend([
                    {"utterance": resp, "persona": persona_name} for resp in utterances
                ])
            except IndexError:
                continue

    # Structure the output
    output = {
        "seed_utterances": seed_utterances,
        "generated_utterances": all_generated_responses,
        "personas": used_personas,
        "metadata": {
            "num_personas": num_personas,
            "utterances_per_persona": utterances_per_persona,
            "personas_per_batch": personas_per_batch,
            "total_utterances": len(all_generated_responses)
        }
    }

    return output
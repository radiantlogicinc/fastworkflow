...
def test_workflow_schema_validation():
    ...
    try:
        child_schemas = {
            ...
            "InvalidType": {"Task": {"min_cardinality": 0, "max_cardinality": 5}}
        }
    ...
    try:
        child_schemas = {
            "Project": {"InvalidType": {"min_cardinality": 1, "max_cardinality": 10}}
        }
        WorkItem.WorkflowSchema(workflow_types=types, child_schema_dict=child_schemas)
        assert False, "Should have raised ValueError"
    except ValueError as e:
...
def test_path_operations():
    ...
    assert task.get_absolute_path() == "/Task[index=0]"
...
def test_workitem_id():
    """Test WorkItem ID functionality."""
    ...
    # Test path with ID
    assert child1.get_absolute_path() == "/Task[id=task-1]"
    assert parent._get_child_workitem("Task[id=task-1]") == child1


def test_workitem_id_uniqueness():
    """Test WorkItem ID uniqueness validation."""
    parent = WorkItem(workitem_type="Project")
    child1 = WorkItem(workitem_type="Task", id="task-1")
    parent.add_child(child1)
    
    # Test duplicate ID raises error
    try:
        child2 = WorkItem(workitem_type="Task", id="task-1", parent=parent)
        assert False, "Should have raised ValueError"
    except ValueError as e:
        assert "Work item id 'task-1' is not unique among siblings of type 'Task'" in str(e)
    
    # Test adding child with duplicate ID
    child3 = WorkItem(workitem_type="Task", id="task-1")
    try:
...
def main():
    ...
    tests = [
        ...
        (test_workitem_id, "WorkItem ID"),
        (test_workitem_id_uniqueness, "WorkItem ID Uniqueness"),
    ]

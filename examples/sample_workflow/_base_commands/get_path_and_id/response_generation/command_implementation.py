...
def process_command(session: Session, input: CommandParameters, payload: Optional[dict] = None) -> CommandProcessorOutput:
    ...
    if not input.for_next_workitem:
        return CommandProcessorOutput(workitem_path=active_workitem.path, workitem_id=active_workitem.id)
    ...
    return CommandProcessorOutput(workitem_path=next_workitem.path, workitem_id=next_workitem.id)
...
if __name__ == "__main__":
    ...
    tool_output = tool(session, tool_input)
    print(tool_output)

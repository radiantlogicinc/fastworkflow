...
class InputForParamExtraction(BaseModel):
    ...
    def validate_parameters(cls, session: Session, cmd_parameters: CommandParameters) -> Tuple[bool, str]:
        ...
        if workitem_path in [workitem_type for workitem_type in session.workflow_definition.types]:
        ...
        workitem = session.workflow.find_workitem(workitem_path, 
                                                  ...
                                                  relative_to_root)
        if workitem is None:
            return (False, "workitem path was valid but does not exist in this workflow")

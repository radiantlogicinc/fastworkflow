...
def process_command(session: Session, input: CommandParameters, payload: Optional[dict] = None) -> CommandProcessorOutput:
    ...
    if input.workitem_path in [workitem_type for workitem_type in session.workflow_definition.types]:
        relative_to_root = True
        input.workitem_path = f'//{input.workitem_path}'

    workitem = session.workflow.find_workitem(input.workitem_path, 
                                              input.workitem_id,
                                              relative_to_root)

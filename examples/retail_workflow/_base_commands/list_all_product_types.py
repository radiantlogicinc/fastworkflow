...
class Signature:
    ...
    class Input(BaseModel):
        ...
        # The schema requires at least one field, so we provide a dummy optional
        # field that is excluded from the public docs by default.  The runtime
        # never needs to pass it.
        dummy: int | None = Field(default=None, exclude=True)
    ...
    def generate_utterances(session: fastworkflow.Session, command_name: str) -> List[str]:
        ...
        utterance_definition = fastworkflow.UtteranceRegistry.get_definition(
            workflow.workflow_folderpath
        )
